...
void Intersection::getting_close(){
    ...
    if(motion.go_for_distance(150, 0, motion.obstacle_controller(20))){   //200 or 150
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
        if(crosswalk_data_n > 40){
...
void Intersection::intersection_preparation(){
    if(motion.go_for_distance(400 - preparation_value, 0, motion.obstacle_controller(20))){ //edit: a:
        uart.debug_print("preparation vlue: ");
        uart.debug_println(preparation_value);
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
        if(crosswalk_angle > -5 && crosswalk_angle < 5){
...
void Intersection::angle_correction(){
    int steer = 0;
    correction = crosswalk_angle * 0.75;
    if(correction > 0){
        steer = 90;
    }
    else{
    ...
    if(motion.go_for_angle(not_abs(correction), steer, 25)){
...
void Intersection::left_I(){    //edit:    g:
    if(motion.go_for_distance(150 + crosswalk_angle, 0, motion.obstacle_controller(20))){  //edit : angle correction shift
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
...
void Intersection::left_II(){
    if(motion.go_for_angle(80 , 90, motion.obstacle_controller(40))){
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
        side_detection_parameter = vision.get_side_detection();
...
void Intersection::side_correction(){
    if(side_detection_parameter == 0){
        ...
        if(side_invalid_counter < 2){
            if(motor.get_distance_covered_mm() > 150){  //edit
                motion.stop();
                vTaskDelay(500);
