...
{
    uart_event_t event;
    uint8_t *buffer = (uint8_t *)malloc(21);
    char *position = (char *)&buffer[1];
    size_t length, fail_counter = 0;
    int temp_sign = 0;
    while (true)
    {
        if (xQueueReceive(support_rxqueue, (void *)&event, (TickType_t)1))
        {
            ...
            case UART_DATA:
                ...
                uart_read_bytes(UART_NUM_2, buffer, 20, 4);
                if (buffer[0] == 's' && buffer[19] == 's')
                {
                    if (!modules.cmps_available() || !modules.mpu_available())
                    {
                        if (buffer[1] == 'c' && buffer[18] == 'c')
                        {
                            if (!modules.cmps_available())
                            {
                                if (xSemaphoreTake(cmps_data_mutex, 1) == pdTRUE)
                                {
                                    cmps.cmps_result[0] = atoi(position);
                                    xSemaphoreGive(cmps_data_mutex);
                                }
                            }
                            if (!modules.mpu_available())
                            {
                            }
                        }
                        else
