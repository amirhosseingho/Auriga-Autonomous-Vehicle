...
void Cross_Park::start_to_park(){
    if (motor.get_distance_covered_mm() < sign_distance + 75){    //edit: cover sign distance + VL
        motion.operate(motion.stanley(), motion.obstacle_controller(20));
    }
    else{
        motion.stop();
...
void Cross_Park::check_block(){ 
    if(motor.get_distance_covered_mm() < 300){  //edit: f:
        ...
        if(left_vl < (500 + 300 + 100 + (1.5 * vision.get_distance_from_midline()))){     //edit: Park_Box + width_of_road + VL_from_mid_line + ....    f:   a:
            counter++;
        }
    } 
    else{
...
void Cross_Park::park_I(){
    bool first_move = motion.go_for_distance(685, 100, park_obstacle_controller(0, 25, 50));
    if(first_move){
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
...
void Cross_Park::park_II(){
    if(motor.get_distance_covered_mm() < 500){  //edit:   a:
        motion.operate(go_straight_cross(-8.1), park_obstacle_controller(0, 40, 50));
    }
    else{
        motion.stop();
        vTaskDelay(5000);
...
void Cross_Park::exit_I(){
    if(motor.get_distance_covered_mm() < 400){  //edit: a:
...
void Cross_Park::exit_II(){
    bool first_move = motion.go_for_distance(700, 90, park_obstacle_controller(1, 25, 50));     //edit:
    if(first_move){
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
